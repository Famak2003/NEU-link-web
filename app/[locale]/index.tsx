...
const Index:React.FC<{children: React.ReactNode}> = ({children}) => {
    ...
    return (
        <div className=" relative w-full max-w-[2000px] overflow-hidden h-fit px-[15px] ">
            <nav className={` fixed top-10 left-[2.5%] mx-auto w-[95%] flex justify-between items-center px-[15px] lmobile:px-[30px] h-fit py-[10px] rounded-[20px] z-[99999999] ${scrolled ? "!bg-primary" : "bg-transparent"} nav `}>
                <Link href={"/"} className=" flex justify-center gap-2 mobile:gap-5 items-end ">
                    ...
                    <h1 className={` text-2xl smobile:text-3xl font-thin ${scrolled ? "text-white" : "text-black"} `}>{dict.NEU_LINK}</h1>
                </Link>
                        ...
                        pages.map((obj, idx) => {
                            return(
                                <Link key={idx} href={`/${locale}/${obj.path}`}>
                                    <li className={` text-[15px] ${scrolled ? "text-white" : "text-black"} `} >
                                        {obj.label}
                                    </li>
                                </Link>
                            )
                ...
                {/* <button className={` hidden tab:flex font-thin text-[18px] h-[90%] py-[15px] px-[15px] rounded-[18px] text-white bg-primary `}>
                 ...
                 <a 
                        ...
                        className={` hidden tab:flex font-thin text-[18px] h-[90%] py-[15px] px-[15px] rounded-[18px] text-white bg-primary `}
                    >
                        {dict.VISIT_NEAR_EAST_UNIVERSITY}
                    </a>
